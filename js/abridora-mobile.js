(() => {
    ...
    function getElements() {
        return {
            ...
            procesoSelect: document.getElementById('abridora-mobile-proceso'),
    ...
    function buildPassesHtml(record) {
        ...
        const proceso = TintoreriaUtils.escapeHtml(record.abridora_proceso || '');
        ...
        const metaParts = [turno, operario, proceso].filter(Boolean).join(' · ');
        const timeLabel = fin
            ? `${inicio} → ${fin}`
            : `${inicio} → en curso...`;
    ...
    function clearFormFields() {
        const els = getElements();
        if (els.procesoSelect) els.procesoSelect.value = '';
    ...
    function populateForm() {
        ...
        if (openRecords.length > 0) {
            if (els.procesoSelect) els.procesoSelect.value = commonValue(openRecords, 'abridora_proceso');
    ...
    async function handleInicio() {
        const els = getElements();
        const proceso = String(els.procesoSelect ? els.procesoSelect.value : '').trim();
        ...
        if (!proceso) {
            showToast('Selecciona el proceso antes de registrar el inicio.');
            if (els.procesoSelect) els.procesoSelect.focus();
            return;
        }
    ...
    async function executeFinWithRouting(siguienteProceso, routingChanges) {
        const els = getElements();
        const ahora = TintoreriaUtils.formatProcessDateTime(new Date());
    ...
    function bindEvents() {
        ...
        if (els.finOptSecado) {
            els.finOptSecado.addEventListener('click', () => {
                executeFinWithRouting('SECADO', { secado_estado: 'X PROCESAR' });
            });
        }

        if (els.finOptRama) {
            els.finOptRama.addEventListener('click', () => {
                executeFinWithRouting('RAMA ACABADO', {
